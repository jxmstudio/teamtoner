...
- **CMS:** Sanity studio embedded at `/studio` (`sanity.config.ts`, schemas in `sanity/schemaTypes/`). All client-editable content is CMS-driven through `lib/data.ts` (async getters, Sanity-first with fixture fallback while `NEXT_PUBLIC_SANITY_PROJECT_ID` is unset/empty): listings, videos, testimonials, guides, suburbs, plus the `siteSettings` singleton, which deep-merges over the `siteConfig` defaults via `getSiteConfig()` — render client-editable values through `await getSiteConfig()`, never `siteConfig` directly. The root layout exports `revalidate = 60` so studio edits reach every page. The header/footer are hidden on `/studio` and `/password` by `components/hide-on-gate.tsx`. See README "Sanity CMS" for setup.
