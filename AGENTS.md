...
- **CMS:** Sanity studio embedded at `/studio` (`sanity.config.ts`, schemas in `sanity/schemaTypes/`). Listings + videos are CMS-driven through `lib/data.ts` (async getters, Sanity-first with fixture fallback while `NEXT_PUBLIC_SANITY_PROJECT_ID` is unset/empty). CMS pages export `revalidate = 60`. See README "Sanity CMS" for the one-time project setup.
